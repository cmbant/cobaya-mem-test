...
def extract_leak_summary(heaptrack_file, container_name):
    """Extract and parse memory leak information."""
    ...
    for i, line in enumerate(lines):
        ...
        leak_match = re.match(r'^([0-9.]+[KMGT]?B?) leaked over ([0-9,]+) calls from$', line.strip())
        if leak_match:
            ...
            if i + 1 < len(lines):
                ...
                # Extract function name (remove leading/trailing whitespace and prefixes)
                func_name = func_line
                
                # Check if this is a camblib.so function
                is_camblib = False
                if i + 2 < len(lines) and 'camblib.so' in lines[i + 2]:
                    is_camblib = True
...
def generate_leak_summary(heaptrack_file, container_name):
    ...
    with open(summary_file, 'w') as f:
        ...
        for func_name in specific_functions:
            ...
            if matching_leaks:
                f.write(f"\n{func_name}:\n")
                for leak in matching_leaks:
                    f.write(f"  {leak['size']} leaked over {leak['calls']} calls\n")
            else:
